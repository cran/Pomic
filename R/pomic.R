		...
		fullmsd=FALSE,  plotting=FALSE,...) 
{
  ...
  d1<-density(object1,bw=bw1,from=r[1],to=r[2],n=nrange)
	...
	if(fullmsd){
		...
		rownames(z) <- c("POMIC","overlap","KLdiv","MSD","CrossMSD")
	}else{
		z <- matrix(NA, nrow=4, ncol=1)
		rownames(z) <- c("POMIC","overlap","KLdiv","MSD")
	}
	...
	z[1,] <-  - sum ( log(pdf2(object1)) )       #"raw" POMIC
  z[2,] <- ( r1[1]<=r2[2] & r1[2]>=r2[1] )		 #overlap
	...
	if(plotting)
	{
		...
		text(0.5,0.5,paste("Information about the density function: \n bw=",round(bw1,4),"\n lower bound=",round(r[1],3),"\n upper bound=",round(r[2],3),"\n n points=",nrange,"\n\n Pomic score=",round(z[1,],3),sep=" "))
				
		on.exit(par(old.par))
	}
	pomic<-list(field_data=object1,sim_data=object2,kernel_estimator=list(bw=bw1,eps=eps,range=r, nrange=nrange),result=z)
	pomic
}
